...
|-----------------------------------------------------------------------------|--------|
| 전체 소요 시간 (```elapsed_time```)                                               |        |
| RAG top-k retrieval 소요 시간 (```retrieve_top_k_time```)                       |        |
| OpenAI API 소요 시간 (```openai_api_time```)                                    |        |
| 사용자 질문 (컬럼 ```test_query```) 에 대한 GPT 모델의 응답 (```assistant_answer```) 의 관련성 |        |
| GPT 모델의 응답 (```assistant_answer```) 의 추가 질문 제안 여부                           |        |
| 네이버 스마트스토어 관련 질문인지 여부 파악의 정확도                                               |        |
