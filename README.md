...
* [4. 전체 구조 및 코드 파일 설명](#4-전체-구조-및-코드-파일-설명)
* [5. 성능 평가 (시스템 평가 및 검증)](#5-성능-평가-시스템-평가-및-검증)
  * [5-1. 평가를 위한 질문 리스트 (100개)](#5-1-평가를-위한-질문-리스트-100개) 
  * [5-2. 평가 코드](#5-2-평가-코드) 
  * [5-3. 평가 결과](#5-3-평가-결과)
* [6. 논의 사항 및 한계점](#6-논의-사항-및-한계점)
  * [6-1. 사용자 경험 (UX) 측면에서 개선이 필요한 부분 (= 실패 사례)](#6-1-사용자-경험-ux-측면에서-개선이-필요한-부분--실패-사례)
  * [6-2. 기타](#6-2-기타)
* [7. 재현 가능성 (실행 가이드)](#7-재현-가능성-실행-가이드)
...
* chat history 를 초기화한 다음에, 다음 항목을 테스트한다.
* [최종 평가 결과 상세 (스마트스토어 **관련** 질문 100개)](evaluation_result_details.csv)
* [최종 평가 결과 상세 (스마트스토어 **무관** 질문 100개)](evaluation_result_details_not_related.csv)

### 5-1. 평가를 위한 질문 리스트 (100개)

* 네이버 스마트스토어 한글 FAQ 데이터의 질문 (총 2,717건) 을 파일로 저장한 후, 해당 파일 기반으로 **네이버 스마트스토어 관련 질문** 추출 요청
  * **ChatGPT-5.2 Thinking** 모델 사용 (OpenAI API가 아닌, ChatGPT 웹사이트를 통해 ChatGPT 실행)
* 프롬프트 **(스마트스토어 관련 질문 생성)**
...
* 프롬프트 **(스마트스토어 무관 질문 생성)**
  * 스마트스토어 관련 질문 생성 후에 바로 이어서 생성 

```
마찬가지 방법으로, 이번에는 스마트스토어와 관련 없는 질문 100개를 생성해 줘
```

* [평가용 질문 리스트 (100개, 스마트스토어 **관련**)](evaluation_question_list.txt)
* [평가용 질문 리스트 (100개, 스마트스토어 **무관**)](evaluation_question_list_not_related.txt)
...
* **1.** 네이버 스마트스토어 **관련** 질문 100개 대상

| 테스트 항목 (테스트 결과 상세 csv 파일의 컬럼명)                                              | 테스트 결과                                             |
|-----------------------------------------------------------------------------|----------------------------------------------------|
| 전체 소요 시간 (```elapsed_time```)                                               | **평균 3.61초** (GPT 응답한 케이스 **평균 4.05초**), 최대 19.38초 |
| RAG top-k retrieval 소요 시간 (```retrieve_top_k_time```)                       | **평균 0.07초**, 최대 0.59초                             |
| OpenAI API 소요 시간 (```openai_api_time```)                                    | **평균 3.98초**, 최대 19.32초                            |
| '관련성 여부' (관련 **있음**) 파악의 정확도                                                | **89.0%** (89 / 100)                               |
| 사용자 질문 (컬럼 ```test_query```) 에 대한 GPT 모델의 응답 (```assistant_answer```) 의 관련성 | **100.0%** (89 / 89)                               |
| GPT 모델의 응답 (```assistant_answer```) 의 추가 질문 제안 여부                           | **100.0%** (89 / 89)                               |

* **2.** 네이버 스마트스토어 **무관** 질문 100개 대상

| 테스트 항목 (테스트 결과 상세 csv 파일의 컬럼명)                        | 테스트 결과                 |
|-------------------------------------------------------|------------------------|
| 전체 소요 시간 (```elapsed_time```)                         | **평균 0.04초**, 최대 0.61초 |
| RAG top-k retrieval 소요 시간 (```retrieve_top_k_time```) | **평균 0.04초**, 최대 0.61초 |
| '관련성 여부' (관련 **없음**) 파악의 정확도                          | **100.0%** (100 / 100) |

## 6. 논의 사항 및 한계점

### 6-1. 사용자 경험 (UX) 측면에서 개선이 필요한 부분 (= 실패 사례)

**1.** 사용자가 처한 구체적 상황을 제시하고, 이 상황을 LLM이 해석하여 적절한 답변하기 어려움

* 예시
  * ```내가 만 29세인데 네이버 스마트스토어에 가입할 수 있어?``` 라는 질문에 대해 적절한 답변 불가 [(참고: 데모 1 이미지)](images/demo_1.PNG)
* 원인
  * **사용자 질문의 embedding 과 DB 내부 질문의 embedding 의 유사도를 계산** 하는 RAG retrieval 알고리즘 특성상, 주어진 상황으로부터 추론이 필요한 내용은 관련 질문과 높은 유사도를 보장하지 못함
* 해결 방법
  * **1)** 해석/추론이 필요한 질문을 DB에 추가하거나,
  * **2)** 추론이 필요한 상황을 포함한 질문과 DB 내부 관련 질문의 유사도가 높아지도록 추가 학습하는 메커니즘 필요 

**2.** 대화 맥락이 있는 상황에서, 가리키는 것이 뻔함에도 '스마트스토어' 등 핵심 어휘 생략으로 '관련 없는 질문'으로 판단

* 예시
  * ```혹시 스마트스토어 가입 절차나 필요한 서류에 대해 안내해 드릴까요?``` 라는 챗봇의 답변 이후, 사용자가 ```필요한 서류 안내해줘``` 라고 질문한 결과 **스마트스토어 무관 질문** 으로 판단 [(참고: 데모 1 이미지)](images/demo_1.PNG)
* 원인
  * 사용자 원본 질문을 **아무런 전후 맥락 없이** RAG 시스템에 입력하기 때문
* 해결 방법
  * 직전 대화 등 전후 맥락을 RAG 시스템 입력에 추가  

**3.** 질문에 대한 적절한 정보가 DB에 있음에도 답변 실패

* 예시
  * 사용자가 ```네이버 스마트스토어에서 상품을 등록하는 방법을 알려줘``` 라고 질문한 결과 **정확한 답변이 어려움** [(참고: 데모 2 이미지)](images/demo_2.PNG)
  * Q&A DB 중 분명히 ```139. ★ 상품등록하는 방법``` 항목이 있고, 그 항목에 상품 등록 방법이 설명되어 있음
* 원인
  * 다음 요소로 인해 코사인 유사도 계산 시 유사도가 낮다고 판단되었을 것으로 추정  
    * 질문에 포함된 특수문자 ```★```
    * 질문이 의문문이 아닌 평서문 형태로 서술되어 있기 때문
* 해결 방법
  * DB에서 위와 같이 특수문자가 있거나 평서문 형태로 서술된 질문들을 찾아서 의문문 형태로 변경

### 6-2. 기타

* 관련성 여부 판단 정확도 (관련성 있음 → 89%, 관련성 없음 → 100%) 는 높지만, **실제 유저 체감은 이보다 낮은 이유**
  * 해당 테스트용 질문은 ChatGPT 웹사이트에서 ChatGPT-5.2 Thinking 으로 추출한 **가장 그럴듯한** 질문들임
  * 실제 유저 테스트 시나리오는 **가장 그럴듯한 범위를 벗어나는** 경우가 많기 때문

* distance threshold (스마트스토어 FAQ 관련/무관 질문 판단 기준) 최적화
  * 아래 히스토그램은 각각 **스마트스토어 관련/무관 질문** 에 대한, **DB에 있는 모든 질문과의 유사도의 최솟값 (shortest distance)** 의 분포를 나타냄 
  * 아래 히스토그램을 보면, distance threshold 를 **기존 0.3 에서 0.45 정도로 늘리면 '정확도 (Accuracy)' 관점에서 최적** 이 될 수 있다고 판단됨

![image](images/additional_1.PNG)
